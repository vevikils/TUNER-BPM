...
$zipPath = Join-Path $workspace 'Release_Builds\Supreme_Tuner_BPM_v2.2.0_Windows.zip'
...
Copy-Item -Path (Join-Path $workspace 'build\TunerBPMPlugin_artefacts\Release\VST3\Supreme Tuner BPM V.2.2.vst3') -Destination (Join-Path $pkgDir 'VST3') -Recurse -Force
Copy-Item -Path (Join-Path $workspace 'build\TunerBPMPlugin_artefacts\Release\Standalone\Supreme Tuner BPM V.2.2.exe') -Destination (Join-Path $pkgDir 'Standalone') -Force
